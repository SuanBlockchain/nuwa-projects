...
import { CheckCircleIcon, ClipboardDocumentIcon, ArrowTopRightOnSquareIcon } from '@heroicons/react/24/outline';
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-surface-dark border border-border-dark rounded-2xl p-6 md:p-8">
        ...
        {/* Transaction Hash */}
        <div className="mb-6">
          <label className="text-gray-400 text-xs font-medium mb-2 block">Transaction Hash</label>
          <div className="bg-background-dark border border-border-dark rounded-xl p-4 flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={handleCopy}
              className="flex-shrink-0 p-2 hover:bg-surface-dark rounded-lg transition-colors group"
              title="Copy to clipboard"
            >
              {copied ? (
                <CheckCircleIcon className="w-5 h-5 text-green-500" />
              ) : (
                <ClipboardDocumentIcon className="w-5 h-5 text-gray-400 group-hover:text-white" />
              )}
            </button>
          </div>
        </div>
        ...
        {/* Submission Time */}
        <div className="mb-6">
          <label className="text-gray-400 text-xs font-medium mb-2 block">Submitted At</label>
          <div className="bg-background-dark border border-border-dark rounded-xl p-4">
            <span className="text-white text-sm">{formatDate(submitResponse.submitted_at)}</span>
          </div>
        </div>
