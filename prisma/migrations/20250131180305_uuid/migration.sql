/*
  ...
  - Changed the type of `projectId` on the `ProjectKeyword` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
  - Changed the type of `keywordId` on the `ProjectKeyword` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
...
ADD COLUMN     "keywordId" UUID NOT NULL,
